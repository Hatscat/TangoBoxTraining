...
/// </summary>
public class __MotionPointCloud : MonoBehaviour, ITangoDepth
{
	/// <summary>
	...
	private Vector3[] m_previous_points;
	private List<Vector3> m_visible_points;
	private Vector3[,,] m_previous_points_indexed;
	...
	public void Start ()
	{
		...
		m_previous_points = new Vector3[MAX_POINT_COUNT];
		m_previous_points_indexed = new Vector3[MAX_POINT_COUNT, MAX_POINT_COUNT, MAX_POINT_COUNT];
		m_points = new Vector3[MAX_POINT_COUNT];
		m_visible_points = new List<Vector3>();
	...
	public void OnTangoDepthAvailable (TangoUnityDepth tangoDepth)
	{
		...
		if (tangoDepth != null && tangoDepth.m_points != null)
		{
			...
			if (m_pointsCount > 0)
			{
				...
				error = "<size=30>" + frameCount + "</size>";

				if (frameCount > 0)
				{
					//m_previous_points = m_points.Clone() as Vector3[];
					//for (int i = 0; i < m_pointsCount; ++i)
					//m_previous_points_indexed[getI(m_points[i].x), getI(m_points[i].y), getI(m_points[i].z)] = m_points[i];
				}
				...
				for (int i = 0; i < m_pointsCount; ++i)
				{
					...
					//if (frameCount > 0)
					//{
					// index previous point by axis

					//m_previous_points_indexed[getI(m_previous_points[i].x), getI(m_previous_points[i].y), getI(m_previous_points[i].z)] = m_previous_points[i];

					//m_visible_points = new List<Vector3>();
					if (m_visible_points.Count < MAX_POINT_COUNT)// && !IsV3Equal(m_points[i], m_previous_points_indexed[getI(m_points[i].x), getI(m_points[i].y), getI(m_points[i].z)])) // && IsV3Uniq(m_points[i], m_visible_points))
					{
						m_visible_points.Add(new Vector3(m_points[i].x, m_points[i].y, m_points[i].z));
						//error = m_points[i].x + " | " + m_points[i].y + " | " + m_points[i].z;
					}
					//}
				...
				if (m_updatePointsMesh)
				{
					...
					*/
					int[] indices = new int[m_visible_points.Count];
					for (int i = 0; i < m_visible_points.Count; ++i)
					{
						indices[i] = i;
					}

					m_mesh.Clear();
					m_mesh.vertices = m_visible_points.ToArray();
	...
	private bool IsV3Equal (Vector3 a, Vector3 b)
	{
		return a != null && b != null && Vector3.SqrMagnitude(a - b) < 0.0001f;
	}

	private bool IsV3Uniq (Vector3 v, List<Vector3> vecArray)
	{
		for (int i = 0; i < vecArray.Count; ++i)
		{
			if (Vector3.SqrMagnitude(v - vecArray[i]) < 0.0001f)
				return false;
		}
		return true;
	}
	...
	{
		return (int)(Mathf.Max(10f, f + 5f) * (MAX_POINT_COUNT / 10f));
	}

	private void OnGUI ()
	{
		if (GUI.Button(new Rect(Screen.width * 0.02f, Screen.height * 0.4f, Screen.width * 0.2f, Screen.height * 0.2f), "<size=40>Reset</size>"))
		{
			m_visible_points = new List<Vector3>();
			frameCount = 0;
		}
